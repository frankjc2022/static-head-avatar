...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    ############ resize the mask to match the size of the resized image start
    # The code for mask is adpating from https://github.com/graphdeco-inria/gaussian-splatting/issues/127
    resized_mask = None
    ...
    if cam_info.mask:
        ...
        # create and save masked image for debug
        masked_preview = torch.clone(resized_image_rgb)
        # change the color of the unmask area
        # 1/4=0.25, 0.25+0.75=1, preserved the pixel brightness
    ...
    # Below code was for reading image with RGBA and load the mask, but in the latest repo it's gone.
    # loaded_mask = None
    # https://github.com/graphdeco-inria/gaussian-splatting/issues/283
...
def cameraList_from_camInfos(cam_infos, resolution_scale, args):
    ...
    with tqdm(enumerate(cam_infos), total=len(cam_infos)) as t:
        for id, c in t:
            t.set_description("Loading Camera Info - {}".format(c.image_name))
